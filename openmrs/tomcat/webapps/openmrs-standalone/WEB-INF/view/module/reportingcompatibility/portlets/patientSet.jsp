...
<script type="text/javascript">
	...
	<c:if test="${model.droppable && model.patientSetSize == 0}">
		if (localList == null) {
			hideLayer('org.openmrs.module:reportingcompatibility:omod:1.5.4.1');
		}
	</c:if>
	...
	function clearThis() {
		if (localList != null) {
			...
			DWRPatientSetService.clearMyPatientSet(<c:if test="${model.id != null}">function() { refreshList(); hideLayer('org.openmrs.module:reportingcompatibility:omod:1.5.4.1')}</c:if>);
		}
	}
	...
	function handleRefreshList(patients) {
		<c:if test="${model.id != null}">
			if (patients.length == 0) {
				...
				showLayer('org.openmrs.module:reportingcompatibility:omod:1.5.4.1');
			}
		</c:if>
